...
// Qrack.
//
// This file was produced by (Anthropic) Claude based on reduce.cpp (then
// modified by Dan Strano to handle complex and mixed gradients).
//
...
// Reuse REDUCE_HEAD from reduce.cpp pattern:
// Given output index o (over all non-axis dims), compute base offset into a
// along all dimensions except the softmax axis.
#define SOFTMAX_HEAD                                                           \
...
#define SOFTMAX_FWD_LOOP(type)                                                 \
  ...
  for (tcapint j = 0U; j < axis_size; ++j) {                                   \
    po->write(out_base + j * out_stride,                                       \
              std::exp((*pa)[base + j * a_stride] - mx) / s);                  \
  }

#define SOFTMAX_BWD_LOOP(type)                                                 \
  const tcapint axis_size = din.shape[id];                                     \
  const tcapint din_stride = din.stride[id];                                   \
  const tcapint out_stride = out.stride[id];                                   \
  const tcapint dout_stride = dout.stride[id];                                 \
  /* dot = sum(dout * out) along axis */                                       \
  type dot = ZERO_R1;                                                          \
  for (tcapint j = 0U; j < axis_size; ++j) {                                   \
    dot += (*po)[out_base + j * out_stride] *                                  \
           (*pdout)[dout_base + j * dout_stride];                              \
  }                                                                            \
  /* din += out * (dout - dot) */                                              \
  for (tcapint j = 0U; j < axis_size; ++j) {                                   \
    pdin->add(din_base + j * din_stride,                                       \
              (*po)[out_base + j * out_stride] *                               \
                  ((*pdout)[dout_base + j * dout_stride] - dot));              \
  }

#define SOFTMAX_FWD_KERNEL(type)                                               \
  /* n = product of all dims except the softmax axis */                        \
  ...
  pfControl.par_for(0, n, [&](const tcapint &o, const unsigned &cpu) {         \
    SOFTMAX_HEAD                                                               \
    SOFTMAX_FWD_LOOP(type)                                                     \
  });

#define SOFTMAX_BWD_KERNEL(type)                                               \
  const int64_t id = (int64_t)index;                                           \
  const tcapint n = din.get_broadcast_size() / din.shape[id];                  \
  pfControl.par_for(0, n, [&](const tcapint &o, const unsigned &cpu) {         \
    SOFTMAX_BWD_HEAD                                                           \
    SOFTMAX_BWD_LOOP(type)                                                     \
  });
...
#define DEVICE_SWITCH_SOFTMAX_FWD(cpu, gpu)                                    \
...
#define DEVICE_SWITCH_SOFTMAX_BWD(cpu, gpu)                                    \
...
// CPU forward
// ---------------------------------------------------------------------------
static void cpu_softmax_fwd(const tcapint &index, const Tensor &a,
                            Tensor &out) {
  GET_CONST_FLAT_TENSOR(RealTensor, a, pa);
  GET_FLAT_TENSOR(RealTensor, out, po);
  SOFTMAX_FWD_KERNEL(real1)
}
...
static void cpu_softmax_bwd(const tcapint &index, Tensor &din,
  ...
  SOFTMAX_BWD_KERNEL(T3)
}
static void cpu_softmax_bwd_real(const tcapint &index, Tensor &din,
                                 const Tensor &out, const Tensor &dout) {
  cpu_softmax_bwd<RealTensor, RealTensor, real1>(index, din, out, dout);
}
static void cpu_softmax_bwd_complex(const tcapint &index, Tensor &din,
                                    const Tensor &out, const Tensor &dout) {
  cpu_softmax_bwd<ComplexTensor, ComplexTensor, complex>(index, din, out, dout);
}
static void cpu_softmax_bwd_mixed(const tcapint &index, Tensor &din,
                                  const Tensor &out, const Tensor &dout) {
  cpu_softmax_bwd<ComplexTensor, RealTensor, complex>(index, din, out, dout);
}
...
// ---------------------------------------------------------------------------
#if ENABLE_GPU
static void gpu_softmax_fwd(const tcapint &index, const Tensor &a,
                            Tensor &out) {
  DISPATCH_GPU_SOFTMAX_FWD(GpuRealStorage, OCL_API_SOFTMAX);
}

static void gpu_softmax_bwd_real(const tcapint &index, Tensor &din,
                                 const Tensor &out, const Tensor &dout) {
  DISPATCH_GPU_SOFTMAX_BWD(GpuRealStorage, GpuRealStorage,
                           OCL_API_SOFTMAX_BACKWARD_REAL);
}
static void gpu_softmax_bwd_complex(const tcapint &index, Tensor &din,
                                    const Tensor &out, const Tensor &dout) {
  DISPATCH_GPU_SOFTMAX_BWD(GpuComplexStorage, GpuComplexStorage,
                           OCL_API_SOFTMAX_BACKWARD_COMPLEX);
}
static void gpu_softmax_bwd_mixed(const tcapint &index, Tensor &din,
                                  const Tensor &out, const Tensor &dout) {
  DISPATCH_GPU_SOFTMAX_BWD(GpuComplexStorage, GpuRealStorage,
                           OCL_API_SOFTMAX_BACKWARD_MIXED);
}
#endif
...
void softmax(const tcapint &index, const Tensor &a, Tensor &out) {
  validate_all_same_device({&a, &out}, "SoftmaxKernel::softmax_forward");
  if ((a.storage->dtype != DType::REAL) ||
      (out.storage->dtype != DType::REAL)) {
    throw std::invalid_argument("Tensor dtype mismatch in softmax_forward!");
  }
#if ENABLE_GPU
  DEVICE_SWITCH_SOFTMAX_FWD(cpu_softmax_fwd, gpu_softmax_fwd);
#else
  cpu_softmax_fwd(index, a, out);
#endif
}
...
void softmax_grad(const tcapint &index, Tensor &din, const Tensor &out,
  ...
  validate_all_same_device({&din, &out, &dout},
                           "SoftmaxKernel::softmax_backward");
