...
unsigned char* cl_alloc(size_t ucharCount)
{
    ...
    posix_memalign(&toRet, WEED_ALIGN_SIZE,
        ((sizeof(unsigned char) * ucharCount) < WEED_ALIGN_SIZE) ? WEED_ALIGN_SIZE
                                                                  : (sizeof(unsigned char) * ucharCount));
    return (unsigned char*)toRet;
#elif defined(_WIN32) && !defined(__CYGWIN__)
    return (unsigned char*)_aligned_malloc(((sizeof(unsigned char) * ucharCount) < WEED_ALIGN_SIZE)
            ? WEED_ALIGN_SIZE
            : (sizeof(unsigned char) * ucharCount),
        WEED_ALIGN_SIZE);
...
#else
    return (unsigned char*)aligned_alloc(WEED_ALIGN_SIZE,
        ((sizeof(unsigned char) * ucharCount) < WEED_ALIGN_SIZE) ? WEED_ALIGN_SIZE
                                                                  : (sizeof(unsigned char) * ucharCount));
#endif
}
